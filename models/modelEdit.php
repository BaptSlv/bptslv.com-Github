...
require 'config.php';
 ...
 function getDelete($hidden, $userId) {
  $bdd = getBdd();
  $delete = $bdd->prepare('UPDATE Tache SET delete_at = CURRENT_TIMESTAMP WHERE tacheid = ? AND tacheuserid = ?');
...
function getRead() {
  $bdd = getBdd();
  $reads = $bdd->prepare('SELECT * FROM Tache WHERE delete_at IS NULL AND tacheuserid = ? ORDER BY labeltache DESC');
